...
export default function Dashboard() {
  ...
  const stats = [
    ...
    { title: "СИНХРОНИЗАЦИЯ", value: "BRIDGE: OK", color: "border-t-primary" },
  ]
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 px-3 py-1 bg-emerald-50 text-emerald-700 rounded-full border border-emerald-100">
           ...
           <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-600">Real-time Bridge Active</span>
        </div>
      </div>
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <Card className="shadow-sm border-0 rounded-lg overflow-hidden border">
          <CardHeader className="flex flex-row items-center justify-between border-b bg-muted/20 py-4 px-6">
            <CardTitle className="text-sm font-bold flex items-center gap-2">
              <Activity className="h-4 w-4 text-rose-500" /> Состояние Asterisk (AltLinux)
            </CardTitle>
            <Link href="/services" className="text-[10px] font-bold text-primary flex items-center gap-1 uppercase tracking-wider hover:underline">
              Мостик <ArrowRight className="h-3 w-3" />
            </Link>
          </CardHeader>
          <CardContent className="py-8">
            <div className="space-y-4">
              <div className="flex justify-between items-center text-sm">
                <span className="text-muted-foreground">Ядро АТС:</span>
                <span className="font-mono font-medium">Asterisk 20.x</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-muted-foreground">AMI Коннектор:</span>
                <Badge className="bg-emerald-500 text-white border-none">CONNECTED</Badge>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-muted-foreground">Авто-синхронизация:</span>
                <span className="font-mono font-medium text-emerald-600">ВКЛЮЧЕНА (PJSIP)</span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-sm border-0 rounded-lg overflow-hidden border">
          <CardHeader className="flex flex-row items-center justify-between border-b bg-muted/20 py-4 px-6">
            <CardTitle className="text-sm font-bold flex items-center gap-2">
              <Users className="h-4 w-4 text-primary" /> Реальный статус телефонов
            </CardTitle>
            <Link href="/extensions" className="text-[10px] font-bold text-primary flex items-center gap-1 uppercase tracking-wider hover:underline">
              Все абоненты <ArrowRight className="h-3 w-3" />
            </Link>
          </CardHeader>
          <CardContent className="py-6">
            <div className="space-y-3">
              {extensions?.slice(0, 5).map((ext: any) => (
                <div key={ext.id} className="flex items-center justify-between p-3 rounded-lg border bg-muted/5 group hover:bg-muted/10 transition-colors">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] uppercase font-bold text-muted-foreground/60">{ext.status || 'offline'}</span>
                  </div>
                </div>
              ))}
              {(!extensions || extensions.length === 0) && (
                <div className="text-center text-muted-foreground text-sm py-8 italic">Абоненты не найдены</div>
              )}
            </div>
          </CardContent>
        </Card>
